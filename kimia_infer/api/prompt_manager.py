...
class KimiAPromptManager:
    def __init__(self, model_path: str, kimia_token_offset: int, kimia_text_audiodelaytokens: int):
        self.audio_tokenizer = Glm4Tokenizer("/home/hwang/.cache/huggingface/hub/ZhipuAI/glm-4-voice-tokenizer")
    ...
    def tokenize_message(
        ...
        if tokenize_role:#转成机器可理解文本的过程，通过本地读取wav文件投入Glm4Tokenizer来转
            if role == "user":
        ...
        if message["message_type"] == "text":
            ...
            if "audio_tokens" in message:
                ...
                audio_path = message["content"]#由于没有提前录好的文字，所以直接用audio_path来进行读取
                speech_tokens = self._tokenize_audio(audio_path)#获取speech_tokens
            #将audio的信息合并起来
    ...
    def get_prompt(
        ...
        for msg_idx, message in enumerate(messages):
            assert message["role"] in ["user", "assistant"]#获取角色种类
        ...
        if add_assistant_start_msg:#加入助手元素
